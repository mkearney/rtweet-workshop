...
View these slides at [mkearney.github.io/rtweet-workshop](https://mkearney.github.io/rtweet-workshop)
...
Search for one or more keyword(s) (note: implicit `AND` between words)

```{r}
rds <- search_tweets(q = "rstats data science")
rds
```
...
By default, `search_tweets()` returns up to 100 of the most recent matching 
tweets. To return more, set `n` to a higher number (for normal tokens, the rate
limit is 18,000 every fifteen minutes).

```{r}
rstats <- search_tweets(q = "rstats", n = 10000)
rstats
```
...
**PRO TIP #1**: Use `bearer_token()` to increase rate limit to 45,000 per
fifteen minutes.
...
**PRO TIP #2**: Get the firehose for free by searching for tweets posted by
...
**PRO TIP #3**: Use search operators provided by Twitter, e.g.,

filter by language and exclude retweets
...
search geolocation (ex: tweets within 25 miles of Columbia, MO)
...
```{r}
rt <- search_tweets("lang:en", n = 300,
